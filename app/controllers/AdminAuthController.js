...
const createToken = async ( admin ) => {
	try{
		...
		const refreshToken = jwt.sign({ id: admin.id }, process.env.JWT_REFRESH_TOKEN_KEY);
		...
		if( email && password ) {
			...
			if( adminData ){
				...
				if( result ){
					...
					res.status(401).send({ success: false, message: "password in incorrect."});
				}
			} else {
	...
	try{
		...
		if( receiverEmail ){
			...
			if( emailVerify ){
				...
				if( resetCode ){
					let emailMessage = "Hi " + emailVerify?.firstName + " here is your reset password verification code "+ resetCode +".";
					let emailSubject = 'Password Reset Request'
					let toUser = receiverEmail
					let emailSend = await forgetPasswordEmail( toUser, emailSubject, emailMessage );
					...
					if( emailSend ){
						...
						res.status(200).send({ success: true, message: "reset password email with verification is send to you."})
				...
				res.status(401).send({ success: false, message: "email not verified." })
			}
		} else {
...
exports.verifyCode = async ( req, res, next ) => {
	try{
		...
		if( resetCode ){
			...
			if ( code ){
				res.status(200).send({ success: true, verified: true, message: "code verified."});
			} else {
				res.status(400).send({ success: false, verified: false, message: "code not found."})
			}
		} else {
			res.status(400).send({ success: false, message: "please send verify code."})
		}
	} catch(err){
		console.log('error', err);
		res.status(503).send({ success: false, message: "Server Error." })
...
exports.updatePassword = async ( req, res, next ) => {
	try{
		...
		let hashedPassword = bcrypt.hashSync(password, parseInt(process.env.BCRYPT_SALT));
		...
		if( userData ){
			...
			if ( updatedRows[0] > 0 ){
				...
				res.status(400).send({ success: false, message: "could not reset password."})
			}
		} else {
			res.status(400).send({ success: false, message: "this code is not valid."})
		}
	...
	try{
		...
		if( cityName && cityImg && cityId ){
			...
			if( updatedCity[0] > 0 ){
				...
				res.status(400).send({ success: false, message: 'Could not Update City.' })
			}
		} else {
...
exports.addRestaurant = async ( req, res, next ) => {
	try{
		...
		console.log("restaurants", restuarantImgs)
		if( createdBy && cityId && location && restaurantName && restuarantImgs ){
...
exports.updateRestaurant = async ( req, res, next ) => {
	try{
		...
		if( cityId && restaurantId && restaurantName && restuarantImgs ){
