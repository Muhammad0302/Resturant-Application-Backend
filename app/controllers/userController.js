...
exports.addNewRestaurant = async (req, res) => {
  try {
    ...
    if (stateId && cityId && restaurantName && location) {
      // let resturantExist=await db.restaurants.findOne({location:location.trim(),restaurantName: restaurantName?.trim()})
      // if(!resturantExist){
        ...
        if (addedRestaurants?.dataValues) {
          res.status(200).send({ success: true, message: "Restaurant Added." ,data:addedRestaurants?.dataValues});
        } else {
          res.status(400).send({ success: false, message: "Could not Add Restaurants." });
        }
      // } else{
      //   res.status(400).send({ success: false, message: "Restaurant already exist." });
...
exports.contactus = async (req, res) => {
  try {
    ...
    if (email && subject && message ) {
      let contactUs=await contactUsEmail(name,email,subject,message);
      if (contactUs) {
      await db.contactus.create({
        ...
        res.status(200).send({ success: true, message: "contact us email sent." });
      } else {
        res.status(400).send({ success: false, message: "Can not be sent." });
      }
    } else {
      res.status(400).send({ success: false, message: "Send Proper Data." });
    }
...
const contactUsEmail = (name,from, subject, message) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      var transporter = nodemailer.createTransport({
        service: "gmail",
        auth: {
          user: "rezzlist@gmail.com",
          pass: "owmajghgufjhcevi",
        },
      });

      var mailOptions = {
        from:`Rezzlist <${from}>`,
        to: "rezzlist@gmail.com",
        subject: subject,
        html: ` ${name} <br >${from} <br >${message}`,
        // text: message,
      };
