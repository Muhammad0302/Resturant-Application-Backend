...
let SUCCESSURL = "http://www.rezzlist.com/CongratulationsReservationPurchased";
...
exports.startCheckOut = async (req, res) => {
  try {
    ...
    if (availableReservation) {
      ...
      res.send({
        success: false,
        message: "This reservation is already occupied.",
      });
    }
  } catch (err) {
...
exports.successCheckOut = async (req, res) => {
  try {
    ...
    if (session_id && userId && reservationId) {
      ...
      let reservation = await db.reservations.findOne({
        where: {
          id: reservationId,
        },
      });
      await connectedAccount.createTransfer(
        reservation.userAccount,
        reservation.price
      );
...
exports.cancelCheckOut = async (req, res) => {
  try {
    res
      .status(500)
      .send({ success: false, message: "could not proceed your checkout." });
  } catch (err) {
    console.log("Error", err);
    res.status(503).send({ success: false, message: "Server error." });
  }
